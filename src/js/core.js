...
(function () {
    ...
    function handleBlockDeleteKeydowns(event) {
        ...
        if ((Util.key(event) === Util.keyCode.BACKSPACE || Util.key(event) === Util.keyCode.ENTER) &&
            ...
            if (Util.key(event) === Util.keyCode.BACKSPACE && isEmpty.test(node.previousElementSibling.innerHTML)) {
                // backspacing the begining of a header into an empty previous element will
                // change the tagName of the current node to prevent one
                // instead delete previous node and cancel the event.
                node.previousElementSibling.parentNode.removeChild(node.previousElementSibling);
                event.preventDefault();
            } else if (Util.key(event) === Util.keyCode.ENTER) {
        ...
        } else if (Util.key(event) === Util.keyCode.DELETE &&
            ...
            event.preventDefault();
        } else if (Util.key(event) === Util.keyCode.BACKSPACE &&
                tagName === 'li' &&
    ...
    function handleKeyup(event) {
        ...
        if (Util.key(event) === Util.keyCode.ENTER && !Util.isListItem(node)) {
            tagName = node.tagName.toLowerCase();
